const Cart = {
    ...
    updateUI() {
        ...
        cartItemsContainer.innerHTML = this.items.map((item, index) => {
            ...
            return `<div class="cart-item"><div class="cart-item-info"><span class="cart-item-icon">${this.renderItemImage(item)}</span><div class="cart-item-details"><strong>${name}</strong><small>${companyName}</small><div class="cart-item-price">${this.formatAmount(item.price)}</div></div></div><button class="remove-btn" onclick="Cart.removeFromCart(${index})"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg></button></div>`;
        }).join('');
