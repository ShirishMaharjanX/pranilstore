const Cart = {
    ...
    updateUI() {
        ...
        if (this.items.length === 0) {
            cartItemsContainer.innerHTML = '<div class="empty-cart-state"><svg width="64" height="64" viewBox="0 0 64 64" fill="none" stroke="currentColor" stroke-width="2"><circle cx="32" cy="32" r="24"/><path d="M24 40h16M24 32h16M24 24h8"/></svg><p>Your cart is empty</p></div>';
            cartFooter.style.display = 'none';
        } else {
            cartFooter.style.display = 'block';
            cartItemsContainer.innerHTML = this.items.map((item, index) => `<div class="cart-item"><div class="cart-item-info"><span class="cart-item-icon">${item.image || '📦'}</span><div class="cart-item-details"><strong>${item.name}</strong><small>${item.companyName}</small><div class="cart-item-price">NPR ${item.price.toLocaleString()}</div></div></div><button class="remove-btn" onclick="Cart.removeFromCart(${index})"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg></button></div>`).join('');
