const Admin = {
    ...
    async renderCompaniesManagement(content) {
        ...
        const companyRows = companies.length === 0
            ...
            : companies.map(company => {
                ...
                return `<div class="company-item-admin"><div class="company-item-header"><span style="width:42px;height:42px;border-radius:50%;background:linear-gradient(135deg,#0f172a,#1e3a8a);color:#fff;display:inline-flex;align-items:center;justify-content:center;font-size:0.8rem;font-weight:700;">${initials}</span><div class="company-item-info"><h3>${company.name}</h3><p>${company.products.length} Products</p></div><div class="company-item-actions"><button class="btn-icon" onclick="Admin.showEditCompanyModal(${company.id})" title="Edit"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button><button class="btn-icon" onclick="Admin.deleteCompany(${company.id})" title="Delete"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg></button></div></div></div>`;
            }).join('');

        content.innerHTML = `<div class="admin-section"><div class="section-header"><div><h2>Companies Management</h2><p class="section-subtitle">Manage your partner companies</p></div><button class="btn-primary" onclick="Admin.showAddCompanyModal()">Add Company</button></div><div class="companies-list">${companyRows}</div></div>`;
    },
                ...
                : companies.map(company => {
                    ...
                    const productRows = (company.products || []).length === 0
                        ...
                        : (company.products || []).map(product => {
                            ...
                            return `<tr><td><div class="product-cell">${imageCell}<span>${product.name}</span></div></td><td>${product.gram}</td><td><strong>NPR ${product.price.toLocaleString()}</strong></td><td><div class="table-actions"><button class="btn-icon-small" onclick="Admin.editProduct(${company.id}, ${product.id})">Edit</button><button class="btn-icon-small btn-danger" onclick="Admin.deleteProduct(${company.id}, ${product.id})">Delete</button></div></td></tr>`;
                        }).join('');
                    return `<div class="products-company-section"><div class="company-section-header"><div class="company-section-title"><span style="width:42px;height:42px;border-radius:50%;background:linear-gradient(135deg,#0f172a,#1e3a8a);color:#fff;display:inline-flex;align-items:center;justify-content:center;font-size:0.8rem;font-weight:700;">${initials}</span><h3>${company.name}</h3></div><button class="btn-secondary" onclick="Admin.showAddProductModal(${company.id})">+ Add Product</button></div><div class="products-table"><table><thead><tr><th>Product</th><th>Weight</th><th>Price</th><th>Actions</th></tr></thead><tbody>${productRows}</tbody></table></div></div>`;
                }).join('');
            content.innerHTML = `<div class="admin-section"><div class="section-header"><div><h2>Products Management</h2><p class="section-subtitle">Add, edit, and manage product inventory</p></div></div>${companySections}</div>`;
        } catch (err) {
    ...
    async renderOrdersManagement(content) {
        ...
        try {
            ...
            content.innerHTML = `<div class="admin-section"><div class="section-header"><div><h2>Orders Management</h2><p class="section-subtitle">View and manage customer orders</p></div></div><div class="orders-table"><table><thead><tr><th>Order ID</th><th>Customer</th><th>Items</th><th>Total</th><th>Date</th></tr></thead><tbody>${orders.length === 0 ? '<tr><td colspan="5" style="text-align:center;padding:2rem;color:#999;">No orders yet</td></tr>' : orders.map(order => `<tr><td><code class="order-id-code">${order.id}</code></td><td><strong>${order.customer?.name || 'N/A'}</strong><br><small>${order.customer?.phone || ''}</small></td><td>${order.items?.length || 0} items</td><td><strong>NPR ${parseFloat(order.total || 0).toLocaleString()}</strong></td><td>${new Date(order.createdAt).toLocaleDateString()}</td></tr>`).join('')}</tbody></table></div></div>`;
        } catch (err) {
    ...
    async saveEditedProduct() {
        ...
        try {
            await StorageManager.updateProduct(productId, { name: product.name, price: product.price, gram: product.gram, stock: product.stock, image: product.image });
            this.closeEditModal();
    ...
    closeEditModal() { document.getElementById('editModal').classList.remove('active'); this.currentEditingProduct = null; this.currentEditingCompany = null; this.productImageBase64 = null; },
    ...
    handleFiles(files, previewId) {
        ...
        reader.onload = (e) => {
            ...
            if (isCompanyImage) { this.companyImageBase64 = e.target.result; } else { this.productImageBase64 = e.target.result; }
            ...
            const previewContainer = document.getElementById(previewId);
            previewContainer.innerHTML = `<div class="image-preview-container"><img src="${imageData}" alt="Preview" class="image-preview"><br><button type="button" class="clear-image-btn" onclick="${clearFn}">Clear Image</button></div>`;
        };
