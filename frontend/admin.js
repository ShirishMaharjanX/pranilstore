const Admin = {
    ...
    login() { const password = document.getElementById('adminPassword').value; const errorEl = document.getElementById('adminLoginError'); if (password === StorageManager.ADMIN_PASSWORD) { this.isLoggedIn = true; StorageManager.setAdminLoggedIn(true); this.closeAdminLoginModal(); showNotification('Admin access granted', 'success'); const panel = document.getElementById('adminPanel'); panel.classList.add('active'); this.renderAdminHeader(); this.showView('overview'); } else { errorEl.textContent = 'Incorrect password'; errorEl.classList.add('visible'); document.getElementById('adminPassword').value = ''; } },
