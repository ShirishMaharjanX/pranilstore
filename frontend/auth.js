const CustomerAuth = {
    ...
    async loadDashboardData() {
        ...
        else ordersContainer.innerHTML = orders.slice(-10).reverse().map(order => `<div class="dashboard-order-card"><div class="order-header"><span class="order-id">Order #${order.id}</span><span class="order-date">${new Date(order.createdAt).toLocaleDateString()}</span></div><div class="order-items">${(order.items || []).map(item => `<div class="order-item"><span>${item.image || '📦'} ${item.name}</span><span>NPR ${item.price.toLocaleString()}</span></div>`).join('')}</div><div class="order-total"><strong>Total:</strong><strong>NPR ${parseFloat(order.total || 0).toLocaleString()}</strong></div></div>`).join('');
    },
